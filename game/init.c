...
i32 do_early_init(i32 argc, const char *const *argv, struct main_ctx *ctx)
{
    ...
    s_log_info("Log setup OK. Starting early init...");
    ...
    ctx->r = r_ctx_init(ctx->win, R_TYPE_SOFTWARE, 0);
    if (ctx->r == NULL)
...
i32 do_gui_init(struct gui_ctx *gui, const struct main_ctx *main_ctx)
{
    ...
    gui->mmgr = menu_mgr_init(&menu_manager_cfg,
        main_ctx->r, main_ctx->keyboard, main_ctx->mouse);
    if (gui->mmgr == NULL)
...
void do_main_cleanup(struct main_ctx *ctx)
{
    ...
    if (ctx->r != NULL) r_ctx_destroy(&ctx->r);
